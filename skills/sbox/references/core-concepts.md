...
Walk up to a thing and press E. This is how you build doors, buttons, levers, world
pickups, ATMs, vending machines — anything the stock `PlayerController` should be able to
interact with. `ITriggerListener` is for *walking into* things; `IPressable` is for
*deliberately using* them.
...
All members except `Press` have default implementations
(`Scene/Components/Markers/IPressable.cs:13-67`):

| Member | When |
|---|---|
| `bool Press( Event e )` | **Required.** Press began. Return `true` on success |
| `bool CanPress( Event e )` | Default `true`. Gate the press *and* the tooltip |
| `bool Pressing( Event e )` | Every frame while held. Return `false` to cancel |
| `void Release( Event e )` | Press ended. The interface doc says "only if `Press` returned `true`", but the shipped `PlayerController` discards `Press`'s return and tracks/releases the pressed object unconditionally (`PlayerController.Pressing.cs:157,160`; `StopPressing` `:120-133`) — do not rely on the pairing |
| `void Hover( Event e )` | Started looking at it |
| `void Look( Event e )` | Still looking at it — every frame |
| `void Blur( Event e )` | Stopped looking at it |
| `Tooltip? GetTooltip( Event e )` | Default `null`. Return one to show a prompt |

`Event` is `record struct Event( Component Source, Ray? Ray = default )` — `Source` is the
pressing `PlayerController`, so `e.Source.GameObject` is the player.
`Tooltip` is `record struct Tooltip( string Title, string Icon, string Description, bool
Enabled = true, IPressable Pressable = default )`; the controller fills in `Pressable` and
ANDs `Enabled` with `CanPress` before adding it to `PlayerController.Tooltips`.

> **`Press` executes on the pressing CLIENT, not the host.** The whole hover/press
> pipeline runs from `PlayerController.OnUpdate` inside `if ( !IsProxy )`
> (`PlayerController.DefaultControls.cs:41-49`) — it is local input handling. Anything
> authoritative you do in `Press` must go through an `[Rpc.Host]`, exactly as the engine's
> own `Door` does: `IPressable.Press` calls `Toggle( e.Source.GameObject )`, and `Toggle`
> is `[Rpc.Host]` (`Scene/Components/Map/Door.cs:283-290`, `:361-362`).

Wiring on the stock controller (`PlayerController.Input.cs`, `PlayerController.Pressing.cs`):

- `EnablePressing` (default `true`, `Input.cs:38`), `UseButton` (default `"use"`,
  `Input.cs:43`), `ReachLength` (default `130` units, `Input.cs:48`).
- Target selection is `TryGetLookedAt()` (`Pressing.cs:205-266`): a trace from the eye
  along `ReachLength`, retried with radius `0`, `2`, then `4` so small props aren't
  fiddly and you can still reach through gaps. It calls `HitTriggers()` and ignores the
  player's own hierarchy.
- The component is found with `GetComponentsInParent<IPressable>( includeSelf: true )` —
  so the interface may live on a **parent** of the collider that was hit.
- Read `Hovered`, `Pressed` and `Tooltips` off the controller to draw your own prompt UI.
- Other components on the player can intercept via `PlayerController.IEvents`:
  `GetUsableComponent( GameObject )`, `StartPressing`, `StopPressing`, `FailPressing`.
- **`UseLookControls = false` silently disables pressing** — `UpdateLookAt()` is called
  inside that branch.

Full worked example: `patterns-and-examples.md` → *Example 11*.
